...
from string_create.lib import sympy_utils
...
class Generator(object):
    ...
    def __init__(self, flag_fixed,params):
        ...
        if flag_fixed:
            ...
            self.placeholders={}
            self.const = params.constants
            for i in range(len(self.const)):
                self.placeholders[self.const[i]]=sp.Symbol(self.const[i],real=True,nonzero=True)

            #self.placeholders = {"cm": sp.Symbol("cm", real=True, nonzero=True)}

            assert 1 <= len(self.variables)
            self.coefficients = [x for x in self.placeholders.keys()]
            ...
            self.priori_leaf = OrderedDict({})
            for var in params.priori_leaf:
                self.priori_leaf[str(var)] = sp.Symbol(str(var), real=True, nonzero=True)
            ...
            digits = [str(i) for i in range(-5, abs(6))]
            self.words = (
    ...
    def sample_next_pos_ubi(self, nb_empty, nb_ops, rng):
        ...
        for i in range(nb_empty):
            probs.append(
                (self.nl ** i) * self.p1 * self.ubi_dist[nb_empty - i][nb_ops - 1]  # p1一元操作符数量
            )
        for i in range(nb_empty):
            probs.append(
                (self.nl ** i) * self.p2 * self.ubi_dist[nb_empty - i + 1][nb_ops - 1]  # p2二元操作符数量
    ...
    def get_leaf(self, curr_leaves, rng):
        leaf_list = list(self.variables.keys()) + list(self.priori_leaf.keys())
        return [leaf_list[rng.randint(low=0, high=len(self.variables.keys()))]]
    ...
    def _generate_expr(
        ...
        stack = [None]
        nb_empty = 1  # number of empty nodes 空节点的数量
        l_leaves = 0  # left leaves - None states reserved for leaves 左边的叶子 - 没有为叶子保留的状态
        t_leaves = 1  # total number of leaves (just used for sanity check)叶子总数（只是用于合理性检查）
        ...
        for nb_ops in range(nb_total_ops, 0, -1):
            ...
            )  # created empty nodes - skipped future leaves 创建空节点--跳过未来的叶子
    ...
    def add_identifier_constants_fixed(self, expr_list):
        curr = Counter()
        curr["cm"] = 0
        const_d={}
        for i in range(len(expr_list)):
            if expr_list[i] in self.const and expr_list[i] not in const_d:
                const_d[expr_list[i]] = "cm_{}".format(curr["cm"])
                curr["cm"] += 1
        for i in range(len(expr_list)):
            if expr_list[i] in self.const:
                expr_list[i] = const_d[expr_list[i]]
    ...
    def rewrite_sympy_expr(cfg, expr, rewrite_functions=None):
        ...
        for f in rewrite_functions:
            if f == "expand":
                ...
                expr_rw = sympy_utils.simplify(expr_rw, seconds=1)
        return expr_rw
    ...
    def process_equation(self, infix):
        ...
        f = sympy_utils.remove_root_constant_terms(f, list(self.variables.values()), 'add')
        f = sympy_utils.remove_root_constant_terms(f, list(self.variables.values()), 'mul')
    ...
    def TrigonometricNestingCheck(self, f):
        subtreeunion = f.find(sp.sin) | f.find(sp.cos) | f.find(sp.tan) | f.find(sp.asin) | f.find(sp.acos) | f.find(
            sp.atan)
        if len(subtreeunion) == 0:
            return True
        else:
            for subtree in subtreeunion:
                children = subtree.args
                for child in children:
                    checkset = child.find(sp.sin) | child.find(sp.cos) | child.find(sp.tan) | child.find(
                        sp.asin) | child.find(sp.acos) | child.find(sp.atan)
                    if len(checkset) > 0:
                        return False
        return True

    def SyntaxValidationCheck(self, f):  # and
        return self.TrigonometricNestingCheck(f)
    ...
    def generate_equation(self, rng):
        ...
        f_test = self.infix_to_sympy(infix, self.variables, self.rewrite_functions)
        # ReasonableFlag =self.SyntaxValidationCheck(f_test)
        if not (self.SyntaxValidationCheck(f_test)):
            raise ValueErrorExpression("Trigonometric Nesting")
        if (str(f_test).find("x_1") == -1):
            raise ValueErrorExpression("No x_1!")
