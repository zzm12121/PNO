# ==============================================================================
# 多维动力学系统神经过程基础模型
# Neural Process Foundation Model for Multi-dimensional Dynamical Systems
# 
# 功能：基于Neural Process框架学习复杂网络系统的动力学规律
# 特点：
# 1. 支持多维状态空间建模
# 2. 同时学习节点自身演化和节点间相互作用
# 3. 使用HyperNetwork生成动态权重
# 4. 支持IEEE 754浮点数精确编码
# ==============================================================================
...
import adamod  # 改进的Adam优化器

# 自定义模块导入
from data_create.NetworkSystemInstances_new import GeneralDynamics  # 动力学系统生成器
from data_create.lib.Topo import Topo  # 网络拓扑结构
from data_create.lib.InitCondition import InitCondition  # 初始条件设定
from string_create.Creation import Creation  # 字符串创建工具
import nn_models  # 神经网络模型

from generate_data import GenerateData  # 数据生成器
...
# IEEE 754 浮点数编码工具
# 
# 功能：将浮点数转换为二进制表示，用于精确的数值编码
# 参数：
#   - 16位: num_e_bits=5, num_m_bits=10 
#   - 32位: num_e_bits=8, num_m_bits=23
# 
# 作用：提高模型对数值精度的感知能力，特别适用于动力学系统建模
# ==============================================================================
def float2bit(f, num_e_bits=8, num_m_bits=23, bias=127., dtype=torch.float32):
    """
    将浮点数转换为IEEE 754二进制表示
    
    参数:
        f: 输入浮点数张量 [b,n,d]
        num_e_bits: 指数位数 (默认8位)
        num_m_bits: 尾数位数 (默认23位)
        bias: 偏置值 (默认127)
        dtype: 输出数据类型
    ...
    """
    # 符号位处理：0表示正数，1表示负数
    s = (torch.sign(f + 0.001) * -1 + 1) * 0.5  # 交换正负号映射
    s = s.unsqueeze(-1)
    f1 = torch.abs(f)  # 获取绝对值
    
    # 指数位处理
    e_scientific = torch.floor(torch.log2(f1))  # 科学计数法的指数
    # 处理特殊值：无穷大和NaN
    e_scientific[e_scientific == float("-inf")] = -(2 ** (num_e_bits - 1) - 1)
    e_scientific[torch.isnan(e_scientific) == True] = -(2 ** (num_e_bits - 1) - 1)

    e_decimal = e_scientific + (2 ** (num_e_bits - 1) - 1)  # 加偏置
    e = integer2bit(e_decimal, num_bits=num_e_bits)  # 转换为二进制
    
    # 尾数位处理
    f2 = f1 / 2 ** (e_scientific)  # 归一化到[1,2)区间
    m2 = remainder2bit(f2 % 1, num_bits=bias)  # 提取小数部分
    fin_m = m2[:, :, :, :num_m_bits]  # 截取所需尾数位数
...
# 编码器网络类 (Encoder)
# 
# 功能：将上下文数据编码为潜在表示，支持多维状态空间
# 
# 输入数据包括：
# - t_context: 时间点 [b, #points, 1]  
# - i_context: 节点索引 [b, #points, 1]
# - x_i_context: 目标节点状态 [b, #points, state_dim]
# - x_j_set_context: 邻居节点状态集合 [b, m, state_dim]
# - points_info_context: 点信息索引 [m]
# 
# 输出：潜在变量的概率分布 Normal(z_mean, z_logsigma)
# ==============================================================================
...
class Encoder(nn.Module):
    def __init__(
        ...
        # 时间编码第二层：深度特征提取，使用残差连接
        self.encode_t_2 = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),      # 层归一化稳定训练
            nn.LeakyReLU(inplace=True),    # 激活函数
            nn.Linear(hidden_dim, hidden_dim),
        )

        # ==============================================================================
        # 目标节点状态编码器 - 处理不同维度的节点状态
        # ==============================================================================
        # 多维度编码器列表：为每个可能的状态维度创建专用编码器
        self.encode_x_i_1_enc_multidim = nn.ModuleList([])
        for i in range(max_dim):
            # 第i个编码器处理(i+1)维状态空间
            self.encode_x_i_1_enc_multidim.append(
                nn.Sequential(
                    nn.Linear(int((i + 1) * precision), hidden_dim),  # 输入维度 = 状态维度 × 精度位数
        ...
        # Phi函数第二层：结合目标节点和邻居节点信息
        self.encode_x_j_set_phi_2 = nn.Sequential(
            nn.Linear(hidden_dim + hidden_dim, hidden_dim),  # 拼接目标节点和邻居节点特征
            nn.LayerNorm(hidden_dim),
            nn.LeakyReLU(inplace=True),
            nn.Linear(hidden_dim, hidden_dim),
        )
        ...
        self.encode_set_transformer = nn_models.SetTransformer(hidden_dim, 10, hidden_dim,
...
# 
# 功能：基于上下文学习动力学系统的映射关系
# 
# 核心思想：
# 1. 编码器：将上下文数据编码为潜在表示 z
# 2. 解码器：基于 z 和查询点预测输出
# 3. HyperNetwork：动态生成网络权重，实现few-shot学习
# 
# 支持两种类型：
# - 'self': 建模节点自身演化 dx_i/dt = f(x_i)  
# - 'interaction': 建模节点间相互作用 dx_i/dt = g(x_i, x_j)
# ==============================================================================
...
class CNP(nn.Module):
    def __init__(
            ...
            precision,      # 数值精度位数
            hidden_dim,     # 隐藏层维度  
            encoder=None,   # 编码器 (可选，为None时创建新的)
            type='self',    # CNP类型：'self' 或 'interaction'
            max_dim=5,      # 支持的最大状态维度
    ):
        super().__init__()
        ...
        # ==============================================================================
        # HyperNetwork解码器 - 动态生成神经网络权重
        # 
        # 核心思想：
        # 1. 基于潜在变量z动态生成网络权重和偏置
        # 2. 实现对不同动力学系统的快速适应 (few-shot learning)
        # 3. 包含非线性层和线性层，增强模型表达能力
        # ==============================================================================
        
        # 确定输入维度：self类型用z，interaction类型用z+权重嵌入
        if self.type == 'self':
...
class CNPFoundationModel(nn.Module):
    """
    CNP基础模型 - 复杂动力学系统的统一建模框架
    
    核心思想：
    将复杂网络动力学分解为两个互补的部分：
    1. 自身动力学：dx_i/dt = f(x_i) - 节点内在演化规律
    2. 相互作用动力学：dx_i/dt = Σ_j g(x_i, x_j) - 节点间相互影响
    
    完整动力学方程：dx_i/dt = f(x_i) + Σ_j g(x_i, x_j)
    
    优势：
    - 模块化设计：分别学习两种不同类型的动力学
    - 权重共享：interaction CNP复用self CNP的权重嵌入
    - 通用性强：适用于各种复杂网络系统
    """
    def __init__(
            self,
            precision=32,    # 数值精度：32位浮点数
        ...
        # 共享编码器设计：两个CNP使用独立的编码器以增强表达能力
        encoder = None  # 设为None让每个CNP创建自己的编码器
        
        # 自身动力学CNP：建模节点的内在演化 f(x_i)
        self.CNP_self = CNP(precision, hidden_dim, encoder=encoder, type='self', max_dim=max_dim)
        
        # 相互作用动力学CNP：建模节点间的相互影响 g(x_i, x_j)
    ...
    def forward(self, x_context, x_target_self, x_target_interaction, x_target_total, points_info, state_dim,
                mu_std_target=None):
        """
        前向传播：联合学习自身动力学和相互作用动力学
        
        参数:
            x_context: 上下文数据 (时间、节点状态、邻居信息等)
            x_target_self: 自身动力学目标数据 (输入状态, 真实自身演化)
            x_target_interaction: 相互作用目标数据 (节点对状态, 真实相互作用)
            x_target_total: 总体动力学目标 (完整的dx/dt)
            points_info: 点信息索引
            state_dim: 状态空间维度
            mu_std_target: 标准化参数 (均值, 标准差) - 可选
            
        返回:
            loss_details: 包含各项损失的详细信息
        """
        
        # ==============================================================================
        # 阶段1：自身动力学建模 - 学习 f(x_i)
        # ==============================================================================
        if mu_std_target is None:
            ...
            # 训练模式：使用数据标准化提升训练稳定性
            res_self = self.CNP_self(x_context, x_target_self, state_dim, 
                                   (mu_std_target[0][0], mu_std_target[1][0]), weights_emb=None)
        
        # ==============================================================================
        # 阶段2：相互作用动力学建模 - 学习 g(x_i, x_j)
        # 关键创新：复用self CNP的权重嵌入，实现知识迁移
        # ==============================================================================
        if mu_std_target is None:
            # 使用self CNP的权重嵌入来指导interaction CNP的学习
            res_interaction = self.CNP_interaction(x_context, x_target_interaction, state_dim, 
                                                 None, weights_emb=res_self['weights_emb'])
        else:
            res_interaction = self.CNP_interaction(x_context, x_target_interaction, state_dim, 
                                                 (mu_std_target[0][1], mu_std_target[1][1]), 
                                                 weights_emb=res_self['weights_emb'])
        
        # ==============================================================================
        # 损失函数计算
        # ==============================================================================
        loss_1 = res_self['loss']['loss']          # 自身动力学损失
        loss_2 = res_interaction['loss']['loss']   # 相互作用动力学损失

        # ==============================================================================
        # 可选的第三个损失：整体一致性约束 (目前未启用)
        # 
        # 核心思想：确保 f(x_i) + Σ_j g(x_i, x_j) = dx_i/dt_total
        # 这个约束可以进一步提高模型的物理一致性，但会增加计算复杂度
        # ==============================================================================
        ...
        # loss_3 = -loss_3.sum(2).mean(0).mean()

        # ==============================================================================
        # 最终损失组合
        # ==============================================================================
        loss = loss_1 + loss_2  # 目前只使用前两个损失项
        # loss = loss_2  # 备选：只训练相互作用部分
        ...
        loss_details = {
            ...
            # 自身动力学损失分解
            'loss_1': {
                'loss_1': res_self['loss']['loss'].item(),      # 总的自身动力学损失
                'kl': res_self['loss']['kl'].item(),            # KL散度正则化项
                'neg_log_p': res_self['loss']['neg_log_p'].item()  # 负对数似然项
            },
            
            # 相互作用动力学损失分解
            'loss_2': {
                'loss_2': res_interaction['loss']['loss'].item(),      # 总的相互作用损失
                'kl': res_interaction['loss']['kl'].item(),            # KL散度正则化项
                'neg_log_p': res_interaction['loss']['neg_log_p'].item()  # 负对数似然项
            },
...
class TrainCNPFoundationModel:
    def __init__(
        ...
        # 模型构建和初始化
        # ==============================================================================
        self.max_dim = 5  # 支持的最大状态维度
        self.model = CNPFoundationModel(
            precision=32,      # 使用32位浮点数精度
        ...
        # ==============================================================================
        # 数据集加载 - 包含多种动力学系统的方程和状态数据
        # ==============================================================================
        # 1维系统：包含22000个动力学系统样本
    ...
    def make_batch(self, data=None, flag_IEEE754=True):
        ...
        while sum(lineno) < sum(list(self.dataset_size.values())):
            ...
            while num_in_batch < batch_size:
                ...
                row, col, edge_weights = s_data['sparse_adj']
                row = row.long()
                col = col.long()
                # add self loop
        ...
        for epoch in range(self.params['num_epochs']):
            ...
            if epoch % 1 == 0:
                torch.save(self.model.state_dict(),
                           "saved_model_train_on_exist_oneNODE_new_hypernet_NP_multidim_11sys+3sys.pkl")
                self.model.eval()
...
# 
# 功能：
# 1. 加载配置文件 (支持Simulation_config, MutualisticInteraction_config, GeneRegulatory_config等)
# 2. 初始化训练模型
# 3. 启动训练流程
# 
# 使用方法：
# python script.py  # 使用默认配置Simulation_config
# ==============================================================================
...
def main(cfg):
    ...
    # 选项2: 从预训练模型继续训练 (取消注释下面这行)
    # trainmodel = TrainCNPFoundationModel(cfg, 
    #     model_path="saved_model_train_on_exist_oneNODE_new_hypernet_NP_multidim_11sys+3sys.pkl")
